// https://github.com/wang-xinyu/tensorrtx/blob/44e797bbde2c35bee5cd1219ebf6660370ca907a/arcface/prelu.cu#L1
#include <stdio.h>
...
int PReluPlugin::initialize() TRT_NOEXCEPT { return 0; }

Dims PReluPlugin::getOutputDimensions(int index, const Dims* inputs,
...
DataType PReluPlugin::getOutputDataType(int index,
...
bool PReluPlugin::isOutputBroadcastAcrossBatch(
...
void PReluPlugin::configurePlugin(const PluginTensorDesc* in, int nbInput,
...
void PReluPlugin::attachToContext(cudnnContext* cudnnContext,
...
const char* PReluPlugin::getPluginVersion() const TRT_NOEXCEPT { return "1"; }

void PReluPlugin::destroy() TRT_NOEXCEPT { delete this; }
...
__global__ void prelu_kernel(const float* input, float* output, int num_elem,
                             int input_size, int fm_size, const float* gamma) {
...
void PReluPlugin::forwardGpu(const float* const* inputs, float* output,
  ...
  assert(cudaMemcpy(dev_gamma, gamma_.data(), sizeof(float) * gamma_.size(),
                    cudaMemcpyHostToDevice) == cudaSuccess);
  prelu_kernel<<<grid_size, block_size>>>(
      inputs[0], output, input_size_ * batchSize, input_size_,
...
PReluPluginCreator::PReluPluginCreator() {
  mPluginAttributes.emplace_back(
      PluginField("gamma", nullptr, PluginFieldType::kFLOAT32, 1));
...
IPluginV2IOExt* PReluPluginCreator::createPlugin(
...
IPluginV2IOExt* PReluPluginCreator::deserializePlugin(
