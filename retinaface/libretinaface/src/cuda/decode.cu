...
size_t DecodePlugin::getSerializationSize() const TRT_NOEXCEPT { return 0; }

int DecodePlugin::initialize() TRT_NOEXCEPT { return 0; }

Dims DecodePlugin::getOutputDimensions(int index, const Dims* inputs,
                                       int nbInputDims) TRT_NOEXCEPT {
  // output the result to channel
  int totalCount = 0;
  totalCount += decodeplugin::INPUT_H / 8 * decodeplugin::INPUT_W / 8 * 2 *
                sizeof(decodeplugin::Detection) / sizeof(float);
  totalCount += decodeplugin::INPUT_H / 16 * decodeplugin::INPUT_W / 16 * 2 *
                sizeof(decodeplugin::Detection) / sizeof(float);
  totalCount += decodeplugin::INPUT_H / 32 * decodeplugin::INPUT_W / 32 * 2 *
                sizeof(decodeplugin::Detection) / sizeof(float);
...
DataType DecodePlugin::getOutputDataType(int index,
...
bool DecodePlugin::isOutputBroadcastAcrossBatch(
...
void DecodePlugin::configurePlugin(const PluginTensorDesc* in, int nbInput,
...
void DecodePlugin::attachToContext(cudnnContext* cudnnContext,
...
const char* DecodePlugin::getPluginVersion() const TRT_NOEXCEPT { return "1"; }

void DecodePlugin::destroy() TRT_NOEXCEPT { delete this; }
...
__global__ void CalDetection(const float* input, float* output, int num_elem,
                             int step, int anchor, int output_elem) {
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    char* data = (char*)res_count + sizeof(float) +
                 count * sizeof(decodeplugin::Detection);
    ...
    // Location
    det->bbox[0] =
        prior[0] + bbox_reg[idx + k * total_grid * 4] * 0.1 * prior[2];
    det->bbox[1] = prior[1] + bbox_reg[idx + k * total_grid * 4 + total_grid] *
                                  0.1 * prior[3];
    det->bbox[2] =
        prior[2] *
        expf(bbox_reg[idx + k * total_grid * 4 + total_grid * 2] * 0.2);
    det->bbox[3] =
        prior[3] *
        expf(bbox_reg[idx + k * total_grid * 4 + total_grid * 3] * 0.2);
    ...
    for (int i = 0; i < 10; i += 2) {
      det->landmark[i] =
          prior[0] +
          lmk_reg[idx + k * total_grid * 10 + total_grid * i] * 0.1 * prior[2];
      det->landmark[i + 1] =
          prior[1] + lmk_reg[idx + k * total_grid * 10 + total_grid * (i + 1)] *
                         0.1 * prior[3];
...
void DecodePlugin::forwardGpu(const float* const* inputs, float* output,
  ...
  for (unsigned int i = 0; i < 3; ++i) {
    num_elem = batchSize * decodeplugin::INPUT_H / base_step *
               decodeplugin::INPUT_W / base_step;
    thread_count = (num_elem < thread_count_) ? num_elem : thread_count_;
    CalDetection<<<(num_elem + thread_count - 1) / thread_count,
                   thread_count>>>(inputs[i], output, num_elem, base_step,
                                   base_anchor, totalCount);
    base_step *= 2;
    base_anchor *= 4;
  }
}

int DecodePlugin::enqueue(int batchSize, const void* const* inputs,
...
IPluginV2IOExt* DecodePluginCreator::createPlugin(
...
IPluginV2IOExt* DecodePluginCreator::deserializePlugin(
